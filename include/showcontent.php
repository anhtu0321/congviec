...
$sql="select id,noidung,han,giaocho,luu,ghichu,file,trangthai,ngaynhap,nguoinhap,ngayxong from congviec where nguoinhap ='$nguoinhap'";
switch($nhom){
	case "tronghan":
		$sql=$sql." and trangthai = 'Đang xử lý' and (DATE_ADD(han, INTERVAL -2 DAY)> CURDATE())";
		break;
	case "gap":
		$sql=$sql." and trangthai = 'Đang xử lý' and (DATE_ADD(han, INTERVAL -2 DAY)<= CURDATE()) and (han >= CURDATE())";
		break;
	case "khonghan":
		$sql=$sql." and (trangthai = 'Đang xử lý') and (han ='')";
		break;
	case "quahan":
		$sql=$sql." and (han <> '') and (trangthai = 'Đang xử lý') and (han < CURDATE())";
		break;
	case "hoanthanh":
		$sql=$sql." and (trangthai ='Hoàn thành')";
		break;
	case "thatbai":
		$sql=$sql." and (trangthai ='Thất bại')";
		break;
	default:
		$sql="";
		break;
...
$sql=$sql." order by id DESC limit $batdau,$soluong";
$tb = mysql_query($sql); 
?>
<div class="tab-pane fade show active" id="tronghan" role="tabpanel">
    ...
    while($rs = mysql_fetch_array($tb)){ $i++; ?>
        <tr <? if($i % 2  == 0) echo "style='background:#FFFFFF;'";?>>
            ...
            <td align="center"><? if($rs["han"]!=""){echo date("d/m/Y",strtotime($rs["han"]));} else echo "Không có hạn";?></td>
